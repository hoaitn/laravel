...
class Generator {
    ...
    public function createModule() {
        //create main folder
        $mainFolder = __DIR__ . '/../app/Modules/' . $this->_moduleName;
        if (!is_dir($mainFolder)) {
            @mkdir($mainFolder);
        }
        //create subfolder
        foreach ($this->_folderList as $folderName) {
            $subFolder = __DIR__ . '/../app/Modules/' . $this->_moduleName . '/' . $folderName;
            if (!is_dir($subFolder)) {
                @mkdir($subFolder);
    ...
    private function generateConfig() {
        ...
        $file = __DIR__ . "/../app/Modules/{$this->_moduleName}/Http/routes.php";
        $fp = fopen($file, "w");
        fwrite($fp, $content);
        fclose($fp);
        ...
        $content = str_replace('{module-name}', strtolower($this->_moduleName), $content);
        $file = __DIR__ . "/../app/Modules/{$this->_moduleName}/module.json";
        $fp = fopen($file, "w");
        fwrite($fp, $content);
        fclose($fp);
    }
    ...
    public function createModel($fieldName, $fieldData) {
        ...
        $modelPath = __DIR__ . "/../app/Modules/{$this->_moduleName}/Models/{$this->_moduleName}.php";
        ...
        if (count($fieldData) > 0) {
            $this->createDBTable($fieldData);
            $this->generateMigrations($fieldData);
        }
    }
    ...
    private function generateMigrations($fieldData) {
        //dump($fieldData);
        return true;
    }
